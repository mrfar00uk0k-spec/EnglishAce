...
export function LexiProvider({ children }) {
  ...
  // Fully clear any previous test's message so Lexi shows nothing until the new test finishes.
  const resetLexi   = useCallback(() => { setMessage(null); setVisible(false); setExpanded(false); setHasNew(false) }, [])

  return (
    <LexiCtx.Provider value={{ visible, expanded, message, hasNew, greeting, dailyTip, showLexi, hideLexi, dismissLexi, toggleLexi, clearNew, resetLexi }}>
      {children}
    </LexiCtx.Provider>
  )
}
