...
export default function Reading({ onFinish, isSingle }) {
  ...
  // Fix #5 & #6: Reading-specific dynamic feedback
  function getReadingStrengths(sc) {
    if (sc >= 80) return ['Strong reading comprehension across all question types', 'Excellent ability to locate specific information', 'Good understanding of both explicit and implicit meaning']
    if (sc >= 60) return ['Good grasp of main ideas in the passage', 'Able to follow the general argument of the text']
    if (sc >= 40) return ['You understood some of the passage content', 'Attempted all questions — persistence is key']
    return ['You completed the reading test']
  }
  function getReadingWeaknesses(sc, wrongCount) {
    if (sc >= 80) return wrongCount > 0 ? ['Some detail-level questions were missed — re-read more carefully'] : []
    if (sc >= 60) return ['Inference questions need more practice', 'Scanning for specific details could be faster']
    if (sc >= 40) return ['Vocabulary gaps may be affecting comprehension', 'Practice reading 1 article daily to build speed', 'Focus on understanding paragraph structure']
    return ['Reading comprehension needs significant practice', 'Build vocabulary with 10 new words daily', 'Start with shorter, simpler texts']
  }
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      {submitted && (() => {
        ...
        return (
          <AssessmentFeedback
          strengths={getReadingStrengths(score)}
          weaknesses={getReadingWeaknesses(score, passage.questions.filter((q,i)=>answers[i]!==q.answer).length)}
