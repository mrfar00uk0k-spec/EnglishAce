...
// Pick 5 questions ordered easy → hard (first 5 from sorted bank)
function getPicedQuestions(pool) {
  // Bank is already ordered: Q1-5 easy, Q6-10 medium, Q11-15 hard
  // Pick 1 easy (1-5), 2 medium (6-10), 2 hard (11-15)
  const easy   = pool.filter((_,i) => i < 5)
  const medium = pool.filter((_,i) => i >= 5 && i < 10)
  const hard   = pool.filter((_,i) => i >= 10)
  const pick = (arr, n) => arr.sort(() => Math.random()-0.5).slice(0,n)
  return [...pick(easy,1), ...pick(medium,2), ...pick(hard,2)]
}

export default function Grammar({ onFinish, isSingle }) {
  const { t, lang } = useLang()
  const isAr = lang === 'ar'
  const questions = useMemo(() => getPicedQuestions(grammarQuestions), [])
