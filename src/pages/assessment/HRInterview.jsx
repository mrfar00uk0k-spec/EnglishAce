  ...
  const toCorrArr = (v) => {
    ...
    return v.map(item => {
      ...
      if (typeof item === 'object' && item !== null) {
        const { category='', issue='', improvement='', example='' } = item
        let str = ''
        if (issue) str += issue
        if (improvement) str += (str ? ' → ' : '') + improvement
        if (example) str += ' (e.g. ' + example + ')'
  ...
  End Recording
      ...
      {phase === 'feedback' && feedback && (
        <AssessmentFeedback
          ...
          lang="en"
          transcript={currentResult?.transcript || savedTranscript || ''}
          mistakes={currentResult?.mistakes || []}
