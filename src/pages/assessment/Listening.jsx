...
export default function Listening({ onFinish, isSingle }) {
  ...
  // ── FINISHED SCREEN ────────────────────────────────────────────
  if (finished) {
    // If time ran out mid-test, average over ALL questions in the test (unanswered = 0),
    // not just the ones the user managed to answer before time expired.
    const totalQuestionCount = questions ? questions.length : results.length
    const avg = totalQuestionCount > 0 ? Math.round(totalScore / totalQuestionCount) : 0
