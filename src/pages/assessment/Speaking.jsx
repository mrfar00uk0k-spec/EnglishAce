...
async function uploadAudioForTranscript(audioBlob) {
  ...
  // Use webm if available, otherwise whatever the browser recorded
...
// ─────────────────────────────────────────────────────────────────────────────
// SpeechRecognition fallback (browser only)
// ─────────────────────────────────────────────────────────────────────────────
...
function buildSRFallback(onPartialUpdate, onFinalToken) {
  ...
  rec.onerror = (e) => {
    ...
    console.warn('[SR fallback] error:', e.error)
  }
  ...
  const getText = () => finalText.trim()

  return { start, stop, getText }
}
...
export default function Speaking({ onFinish, isSingle }) {
  ...
  const isStoppedRef    = useRef(false)   // guard against double-stop
  const isSubmittedRef  = useRef(false)   // guard against double-submit
  const mediaStreamRef  = useRef(null)
  const mediaRecRef     = useRef(null)
  const audioChunksRef  = useRef([])
  const srRef           = useRef(null)
  const srTextRef       = useRef('')      // SR fallback accumulator
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      // Continue without MediaRecorder — SR fallback will still work
    }

    // ── SpeechRecognition (SR) — live visual + fallback transcript ──────
    ...
    // Warm-up delay before enabling Stop button
  ...
  const handleStop = useCallback(async () => {
    ...
    // Collect final audio blob from MediaRecorder
    let audioBlob = null
    ...
    // Stop media stream after recorder is done
    if (mediaStreamRef.current) {
      mediaStreamRef.current.getTracks().forEach(t => t.stop())
      mediaStreamRef.current = null
    }
    ...
    // 1. Try Whisper (server-side) if we have audio data
    if (audioBlob && audioBlob.size > 1000) {
      try {
        transcript = await uploadAudioForTranscript(audioBlob)
        transcriptSource = 'whisper'
        console.log(`[Speaking] Whisper transcript (${transcript.split(' ').length} words):`, transcript)
      } catch (e) {
        console.warn('[Speaking] Whisper failed, falling back to SR:', e.message)
      }
    }

    // 2. SR fallback if Whisper failed or got nothing
    if (!transcript || transcript.length < 3) {
      transcript = srTextRef.current
      transcriptSource = transcript ? 'sr-fallback' : 'none'
      console.log(`[Speaking] SR fallback (${transcript.split(' ').length} words):`, transcript)
    }

    console.log(`[Speaking] Final transcript source=${transcriptSource}, text="${transcript}"`)

    // ── Guard: empty or too short transcript ─────────────────────────────
    if (!transcript || transcript.trim().split(/\s+/).filter(Boolean).length < 3) {
    ...
    try {
      ...
      const result = await evaluateWithAI(topicContext, transcript)
      setFeedback(result)
      setPhase('feedback')
      showLexi({ type: 'speaking', overall: result.overall_score, level: result.level, strengths: result.strengths, weaknesses: result.weaknesses, mistakes: result.mistakes, tips: result.tips })
    } catch (e) {
      console.error('[Speaking] AI evaluation error:', e.message)
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      {phase === 'ready' && (
        <div style={{ textAlign: 'center' }}>
          ...
          <button onClick={startCountdown} style={{ background: 'linear-gradient(135deg,#7c3aed,#8b5cf6)', color: '#fff', border: 'none', borderRadius: 14, padding: '14px 32px', fontSize: '1rem', fontWeight: 700, cursor: 'pointer', boxShadow: '0 6px 20px rgba(124,58,237,0.4)' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 4, margin: '0.75rem 0' }}>
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} style={{ width: 4, borderRadius: 2, background: isReady ? '#8b5cf6' : '#f59e0b', animation: isReady ? `wave 1s ease-in-out ${i * 0.08}s infinite alternate` : 'none', height: isReady ? 20 : 6, transition: 'height 0.3s' }} />
            ))}
          </div>

          {/* Live transcript */}
          <div style={{ background: 'rgba(255,255,255,0.05)', borderRadius: 12, padding: '1rem', margin: '0.75rem 0 1.25rem', minHeight: 55, textAlign: 'left', color: liveText ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.3)', fontSize: '0.9rem', lineHeight: 1.6, maxHeight: 120, overflowY: 'auto' }}>
            {liveText || (isReady ? '🎤 Speak now — your words will appear here...' : '⏳ Initializing microphone...')}
          </div>

          <button onClick={handleStop} disabled={!isReady} style={{ background: isReady ? '#7c3aed' : 'rgba(124,58,237,0.3)', color: '#fff', border: 'none', borderRadius: 12, padding: '12px 28px', fontSize: '0.95rem', fontWeight: 700, cursor: isReady ? 'pointer' : 'not-allowed', transition: 'all 0.4s' }}>
      ...
      {phase === 'unavailable' && (
        <div style={{ textAlign: 'center', padding: '2rem' }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <button onClick={() => { isStoppedRef.current = false; isSubmittedRef.current = false; audioChunksRef.current = []; srTextRef.current = ''; setPhase('ready') }} style={{ background: 'rgba(255,255,255,0.1)', color: '#fff', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 10, padding: '10px 24px', cursor: 'pointer', fontWeight: 600 }}>
              Try Again
            </button>
            <button onClick={() => onFinish(0)} style={{ background: 'rgba(37,99,235,0.3)', color: '#fff', border: '1px solid rgba(37,99,235,0.4)', borderRadius: 10, padding: '10px 24px', cursor: 'pointer', fontWeight: 600 }}>
              Skip →
            </button>
          </div>
        </div>
      ...
      {phase === 'feedback' && feedback && (
        <div style={{ animation: 'pageIn 0.4s ease' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <button onClick={() => onFinish(feedback.overall_score || 0)} style={{ background: 'linear-gradient(135deg,#2563eb,#0ea5e9)', color: '#fff', border: 'none', borderRadius: 12, padding: '13px 32px', fontSize: '0.95rem', fontWeight: 700, cursor: 'pointer' }}>
