...
import { saveSession, loadSession, clearSession, clearOnStart } from '../../utils/session.js'
...
async function evaluateWithAI(topic, transcript, topicObj) {
  const res = await fetch('/api/chat', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ mode: 'speaking', question: topic, transcript, topicEn: topicObj?.en, topicPrompt: topicObj?.prompt }),
  })
...
export default function Speaking({ onFinish, isSingle }) {
  ...
  // Fix #12: If no saved feedback, clear old session to start fresh
  useEffect(() => {
    if (topic) return
    if (!saved?.feedback) clearOnStart('speaking')
    let cancelled = false
...
if (!hasRealAudio) {
  ...
  return
    ...
    try {
      ...
      // Fix #2A: Include topic context so AI can check relevance
      const result = await evaluateWithAI(topicContext, spokenText, topic)
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      {phase === 'feedback' && feedback && (
        <div>
          {/* Fix #2A: Show irrelevant answer warning */}
          {feedback.relevance_warning && (
            <div style={{ background:'rgba(245,158,11,0.1)', border:'1px solid rgba(245,158,11,0.3)', borderRadius:14, padding:'1rem 1.25rem', marginBottom:'1rem', display:'flex', gap:12, alignItems:'flex-start' }}>
              <span style={{ fontSize:'1.2rem', flexShrink:0 }}>⚠️</span>
              <div>
                <div style={{ color:'#f59e0b', fontWeight:700, fontSize:'0.93rem', marginBottom:4 }}>Your answer does not fully address the question</div>
                <p style={{ color:'rgba(255,255,255,0.6)', fontSize:'0.85rem', lineHeight:1.65, margin:0 }}>{feedback.relevance_warning}</p>
              </div>
            </div>
          )}
        <AssessmentFeedback
