...
const welcomeEN = [
  ...
  'Get personalised AI coaching 24/7',
...
// Fix #16: One color per phrase, matching test category colors
const PHRASE_COLORS = [
...
// Fix #16: Animated welcome — equal intervals, different colors per phrase
function AnimatedWelcome({ phrases, colors }) {
  ...
  React.useEffect(() => {
    ...
    }, 3000) // Fix #16: equal 3s interval for every phrase
  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        textShadow: `0 0 24px ${colors[index % colors.length]}44`,
      }}>{phrases[index]}</span>
    </div>
  )
}
...
function useReveal() {
  useEffect(() => {
    ...
    // Safety net: on fast or programmatic scrolls the IntersectionObserver can
    // occasionally miss an element's crossing moment, leaving it stuck at
    // opacity:0 forever. This double-checks anything not yet revealed against
    // its actual position on every scroll so nothing can stay permanently hidden.
    const revealIfInView = () => {
      const vh = window.innerHeight || document.documentElement.clientHeight
      els.forEach(el => {
        if (el.classList.contains('visible')) return
        const rect = el.getBoundingClientRect()
        if (rect.top < vh && rect.bottom > 0) el.classList.add('visible')
      })
    }
...
export default function Home() {
  ...
  // Fix #16: pass full arrays, component rotates with equal intervals
  const welcomePhrases = lang === 'ar' ? welcomeAR : welcomeEN
  const welcomeColors  = PHRASE_COLORS
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh', color: '#fff' }}>
      ...
      {/* ───── FIXED BACKDROP ─────
          Rendered via a portal straight into <body>, bypassing the route
          wrapper's page-transition animation entirely. That wrapper applies
          a CSS transform for its 0.3s enter animation, and any transformed
          ancestor becomes the containing block for position:fixed children —
          which would silently turn "fixed" into "scrolls with the page".
          Portaling to body sidesteps that regardless of how Home is reached
          (fresh load vs. a client-side nav from another page). */}
      {createPortal(
        <div aria-hidden="true" style={{
          position: 'fixed', inset: 0, zIndex: -1,
          background: 'rgb(6,10,22)', overflow: 'hidden', pointerEvents: 'none',
        }}>
          <div className="fixed-bg-grid" style={{ position: 'absolute', inset: 0 }} />
      ...
      <section className="hero-section" style={{
        ...
        <div style={{ position:'relative', maxWidth:820, margin:'0 auto' }}>
          ...
          <div style={{
            ...
            fontSize:'0.83rem', color:'#c4b5fd',
            marginBottom:'1.25rem', letterSpacing:'0.04em', fontWeight:700,
            animation:'float 5s ease-in-out infinite',
          }}>
            <span style={{display:'inline-flex', alignItems:'center', gap:6}}>
              <IconSparkle size={12} color="#c4b5fd" /> {t('hero_platform_badge')}
            </span>
          </div>

          {/* Animated welcome line */}
          <AnimatedWelcome phrases={welcomePhrases} colors={welcomeColors} />
          ...
          <h1 style={{
            ...
            marginBottom:'1.5rem',
            background:'linear-gradient(90deg, #38bdf8, #818cf8, #f472b6, #93c5fd, #38bdf8)',
            backgroundSize:'400% 100%',
            WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent',
            animation:'heroWaveGradient 10s ease-in-out infinite',
          }}>{t('hero_title')}</h1>
  ...
  gap:'2.5rem'
          ...
          <style>{`
            ...
            @media(max-width:560px){
              .preview-inner-resp { grid-template-columns: 1fr !important; }
            }
          `}</style>
        </div>
      </section>
          ...
          <div style={{
            ...
            <div>
              ...
              <div style={{ display:'flex', gap:'0.65rem', flexWrap:'wrap', marginBottom:'1.75rem' }}>
                {['Tell me about yourself.', 'Why should we hire you?', 'How do you handle a difficult customer?'].map((q,i) => (
      ...
      <section className="reveal" style={{ padding:'4rem 1.5rem' }}>
        <div className="why-inner" style={{ maxWidth:1000, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'3rem', alignItems:'center' }}>
          ...
          <div className="reveal-right why-stat-box" style={{
            background:'rgba(255,255,255,0.04)',
  ...
  {t('hero_cta')}
        ...
        <style>{`
          @media(max-width:700px){
            ...
            .why-stat-box{ display:none !important; } /* Fix #9: never show this box on mobile */
          }
