...
export default function HRPractice() {
  ...
  if (done) {
    return (
      <div style={{ background:'rgb(6,10,22)', minHeight:'100vh', color:'#fff', paddingTop:0 }}>
        <div style={{ maxWidth:600, margin:'0 auto', padding:'4rem 1.5rem', textAlign:'center' }}>
          ...
          <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
            <button onClick={() => { setDone(false); setFinalScore(null) }} style={{ background:'rgba(255,255,255,0.08)', color:'#fff', border:'1px solid rgba(255,255,255,0.18)', borderRadius:10, padding:'11px 24px', cursor:'pointer', fontWeight:600 }}>
              🔄 {isAr ? 'أعد التدريب' : 'Practice Again'}
            </button>
    ...
    <div style={{ background:'rgb(6,10,22)', minHeight:'100vh', color:'#fff', paddingTop:0 }}>
      ...
      <div style={{ background:'rgba(37,99,235,0.08)', borderBottom:'1px solid rgba(37,99,235,0.18)', padding:'0.85rem 1.5rem', textAlign:'center' }}>
        <span style={{ color:'#93c5fd', fontSize:'0.85rem', fontWeight:600 }}>
          🎙️ {isAr ? 'تدريب إضافي اختياري — لا يُحسب ضمن اختبار الإنجليزية' : 'Optional Bonus Practice — does not count toward your English test score'}
          {' · '}
