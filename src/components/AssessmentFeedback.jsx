...
import { getResourcesForWeaknesses } from '../data/learningResources.js'
...
// Fix #5 & #6: Different descriptions per test type
const CEFR_DESC_BY_TYPE = {
  speaking: {
    A1: 'You use familiar words and basic phrases. Practice speaking simple sentences every day.',
    A2: 'You handle simple everyday conversations. Work on fluency and common vocabulary.',
    B1: 'You communicate well in most familiar situations. Reduce hesitation and expand vocabulary.',
  ...
  writing: {
    A1: 'You can write simple words and phrases. Practice writing short sentences about daily topics.',
    A2: 'You write short simple texts. Focus on grammar accuracy and sentence variety.',
    B1: 'You write clear texts on familiar topics. Work on paragraph structure and coherence.',
    B2: 'You write with good clarity and detail. Focus on argumentation, vocabulary, and style.',
    C1: 'You write fluently with well-structured ideas. Refine advanced syntax and academic register.',
    C2: 'You write with precision and sophistication. Engage with complex argumentation and tone.',
  },
  reading: {
    A1: 'You can recognise familiar names and simple sentences. Build vocabulary through graded reading.',
    ...
    B2: 'You read complex texts with good understanding. Focus on implicit meaning and inference.',
    C1: 'You read demanding texts including implicit meaning. Engage with academic and formal writing.',
    C2: 'You read with full comprehension and nuance. Explore literature and specialised content.',
  },
  listening: {
    A1: 'You understand slow, clear speech on familiar topics. Practice with basic listening exercises.',
    A2: 'You follow simple conversations spoken clearly. Listen to podcasts designed for learners.',
    B1: 'You follow the main points of clear standard speech. Practice with news and discussions.',
    B2: 'You understand extended speech and most content. Engage with native-speed podcasts and lectures.',
    C1: 'You understand complex speech, idioms, and accents. Engage with specialised audio content.',
    C2: 'You understand all forms of spoken language effortlessly. Listen to academic debates and media.',
  },
  grammar: {
    A1: 'You use basic structures. Start with core verb tenses and simple sentence patterns.',
    A2: 'You use simple tenses and basic connectors. Practice past tense, questions, and prepositions.',
    B1: 'You use a range of tenses with some errors. Focus on conditionals, modals, and reported speech.',
  ...
  hr: {
    A1: 'You can answer basic interview questions with simple phrases. Practice common HR questions.',
    A2: 'You answer simple interview questions. Work on introducing yourself and your experience clearly.',
    B1: 'You handle most interview questions adequately. Improve with the STAR technique and vocabulary.',
...
function getCEFRMeta(level, assessmentType = 'speaking') {
  const base = CEFR_BASE[level] || CEFR_BASE['B1']
  const typeDescs = CEFR_DESC_BY_TYPE[assessmentType] || CEFR_DESC_BY_TYPE.speaking
  const desc = typeDescs[level] || typeDescs['B1'] || 'You are making good progress in English.'
  return { ...base, desc }
}

// Legacy alias kept for any references
const CEFR_META = CEFR_BASE
...
function genericTips(score, assessmentType) {
  if (score >= 80) return [
    'You\'re performing at an excellent level. Practice advanced vocabulary and idiomatic expressions daily.',
    'Consider taking IELTS or TOEFL to certify your proficiency at this level.',
    'Maintain your progress by reading English articles and listening to podcasts regularly.',
  ]
  if (score >= 60) return [
  ...
  return [
    'Focus on building a strong foundation with structured daily lessons.',
    'Practice with EnglishAce every day — consistency is your fastest path forward.',
    'Use Arabic-explained English courses to understand the "why" behind each rule.',
  ]
}
...
const ResourceCard = memo(function ResourceCard({ resource, delay, rm }) {
  ...
  return (
    <div
      className="af-card"
      style={{ background:'rgba(255,255,255,0.03)', backdropFilter:'blur(18px) saturate(160%)', WebkitBackdropFilter:'blur(18px) saturate(160%)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:18, overflow:'hidden', cursor:'pointer' }}
      onClick={handleOpen}
    >
      {/* Fix #1: YouTube Thumbnail */}
      {thumbnail && !imgErr ? (
        <div style={{ position:'relative', width:'100%', aspectRatio:'16/9', overflow:'hidden', background:'rgba(255,255,255,0.04)' }}>
          ...
          {/* Duration badge */}
          <div style={{ position:'absolute', bottom:8, right:8, background:'rgba(0,0,0,0.8)', color:'#fff', fontSize:'0.7rem', fontWeight:700, padding:'2px 7px', borderRadius:4 }}>
            {duration}
          </div>
        </div>
      ) : (
      ...
      <div style={{ padding:'0.9rem 1rem' }}>
        ...
        <h4 style={{ color:'rgba(255,255,255,0.9)', fontSize:'0.88rem', fontWeight:700, lineHeight:1.45, margin:'0 0 6px' }}>{title}</h4>
        <p style={{ color:'rgba(255,255,255,0.42)', fontSize:'0.76rem', lineHeight:1.6, margin:'0 0 10px' }}>{description}</p>
        <div style={{ background:`${color}0d`, border:`1px solid ${color}22`, borderRadius:8, padding:'5px 9px', marginBottom:10 }}>
          <span style={{ color:color, fontSize:'0.7rem', fontWeight:600 }}>Why? </span>
          <span style={{ color:'rgba(255,255,255,0.5)', fontSize:'0.7rem' }}>{reason}</span>
        </div>
        <button
          className="af-btn"
          onClick={(e) => { e.stopPropagation(); handleOpen() }}
          style={{ background:`linear-gradient(135deg,#f87171,#ef4444)`, color:'#fff', border:'none', borderRadius:9, padding:'7px 14px', fontSize:'0.76rem', fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', gap:6, width:'100%', justifyContent:'center' }}
        >
          <IconYouTube size={13}/>
          Watch on YouTube
        </button>
...
export default function AssessmentFeedback({
  ...
  // Tips (AI or generic)
  const finalTips = tips.length > 0 ? tips : genericTips(overallScore || 0, assessmentType)

  // Resources
  const resources = getResourcesForWeaknesses(weaknesses, assessmentType)
  ...
  return (
    <div style={{ maxWidth:800, margin:'0 auto', padding:'0.5rem 0 3rem' }}>
        ...
        <div style={{ padding:'0 1.5rem', marginBottom:'1.5rem' }}>
          ...
          >
            <div style={{ width:48, height:48, borderRadius:12, background:`${color}18`, border:`1px solid ${color}30`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
              <span style={{ fontSize:'1.4rem', fontWeight:900, color }}>{resolvedLevel}</span>
            </div>
            <div>
      ...
      {/* ── CORRECTIONS (HR Interview Fix #3, #14) ───────────────── */}
      {corrections.length > 0 && (
        <RevealSection delay={DELAYS.mistakes + 200} rm={rm}>
          <div style={{ padding:'0 1.5rem', marginBottom:'1.5rem' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'0.65rem' }}>
              {corrections.map((c, i) => {
                ...
                return (
                  <div key={i} className="af-card" style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:16, padding:'1rem 1.1rem' }}>
                    {category && (
                      <div style={{ display:'inline-flex', alignItems:'center', gap:5, background:'rgba(245,158,11,0.12)', border:'1px solid rgba(245,158,11,0.25)', borderRadius:100, padding:'2px 10px', marginBottom:8 }}>
                        <span style={{ color:'#f59e0b', fontSize:'0.7rem', fontWeight:700 }}>{category}</span>
                      </div>
                    )}
                    {issue && <div style={{ color:'rgba(255,255,255,0.85)', fontSize:'0.88rem', fontWeight:600, marginBottom:4 }}>{issue}</div>}
                    {improvement && <div style={{ color:'rgba(255,255,255,0.55)', fontSize:'0.84rem', marginBottom:example ? 6 : 0 }}>{improvement}</div>}
                    {example && (
                      <div style={{ background:'rgba(6,182,212,0.07)', border:'1px solid rgba(6,182,212,0.15)', borderRadius:8, padding:'6px 10px', color:'#67e8f9', fontSize:'0.8rem', fontStyle:'italic' }}>
                        "{example}"
                      </div>
                    )}
                  </div>
      ...
      {/* ── LEARNING RESOURCES ─────────────────────────────────────── */}
      {resources.length > 0 && (
        <RevealSection delay={DELAYS.resources} rm={rm}>
          <div style={{ padding:'0 1.5rem', marginBottom:'1.75rem' }}>
            <SectionHeading>Recommended Arabic Resources</SectionHeading>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(240px, 1fr))', gap:'0.75rem' }}>
              {resources.map((r, i) => (
                <ResourceCard key={r.id} resource={r} delay={DELAYS.resources + i * 80} rm={rm}/>
              ))}
            </div>
          </div>
        </RevealSection>
