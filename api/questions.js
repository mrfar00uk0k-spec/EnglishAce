...
function handleGrammar(req, res) {
  try {
    ...
    const easy     = grammarQuestions.slice(0, easyEnd)
    const medium   = grammarQuestions.slice(easyEnd, medEnd)
    const hard     = grammarQuestions.slice(medEnd)
    ...
    if (picked.length < count) {
      ...
      const remainder = shuffle(grammarQuestions.filter(q => !pickedIds.has(q.id)))
...
function handleVocabulary(req, res) {
  try {
    ...
    const sorted     = [...vocabularyQuestions].sort((a, b) => a.id - b.id)
...
// Returns a randomized subset of listening sentences, ordered so length and
// complexity increase gradually (2 short/simple + 1 longer/medium by default).
...
function handleListening(req, res) {
  try {
    ...
    const sorted    = [...listeningQuestions].sort((a, b) => a.id - b.id)
    const total     = sorted.length
...
function handleReading(req, res) {
  try {
    ...
    const picked = shuffle(readingPassages).slice(0, count)
...
function handleSpeaking(req, res) {
  try {
    ...
    const picked = shuffle(speakingTopics).slice(0, count)
...
function handleHR(req, res) {
  try {
    ...
    const opener = hrQuestions[0] // "Tell me about yourself."
    const rest   = hrQuestions.slice(1)
    const picked = [opener, ...shuffle(rest).slice(0, count - 1)]
