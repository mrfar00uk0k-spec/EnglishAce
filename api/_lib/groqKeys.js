...
export async function groqChat(systemPrompt, userPrompt, opts = {}) {
  ...
  const seed        = Number.isFinite(opts.seed) ? opts.seed : undefined
  ...
  return withKeyRotation(async (key) => {
    ...
    if (seed !== undefined) requestBody.seed = seed // STEP 17 — deterministic seed for stable, reproducible scoring when requested
